...
- ✅ **Cognates generated** for the newly-attested vocabulary via Opus 4.8 (`scripts/generate_cognates_targum_jonathan.py`, scoped to Jonathan-exclusive roots ≥2 occ): 84 genuine roots identified (≈200 proclitic/particle false-positives filtered), +71 net cognate entries → 1,655 total (later deduped to 1,604 in v3.1.1 — removed duplicate `root_syriac` keys). Cost ~$2. Some cognates also gap-filled common roots (e.g. ܩܒܪ "bury") that previously lacked entries.
