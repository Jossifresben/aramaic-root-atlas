#!/usr/bin/env python3
"""Generate Hebrew & Arabic cognates for roots newly attested by Ephrem — Other Works.

Scope: roots whose corpus attestation is EXCLUSIVELY `ephrem_works`
(i.e. the vocabulary Ephrem's non-Nisibis hymn cycles add to the Atlas).
Roots already attested in another corpus are out of scope — they were
coverable before this corpus landed.
...
Requires ANTHROPIC_API_KEY in the environment. Run with:
    ...
    python3 scripts/generate_cognates_ephrem_works.py --max-batches 1   # smoke test
    python3 scripts/generate_cognates_ephrem_works.py --min-occ 2       # full run
"""
...
Ephrem the Syrian's other hymn cycles (e.g. On Faith, Nativity, Against \
Heresies — Classical Syriac, written in Syriac script; presented here in \
normalized Syriac-script form with academic transliteration). MANY of \
these are NOT genuine triliteral roots — common false positives in \
Classical Syriac are:
- Proclitic + word fragments: d- (relative/genitive), w- (and), b- (in), \
  l- (to), k- (like), often stacked
- Pronominal/suffix fragments and enclitic pronouns
...
def find_ephrem_works_exclusive_uncovered(extractor, cognates_data, min_occ):
    sys.path.insert(0, BASE_DIR)
    ...
    for entry in extractor.get_all_roots():
        if set(entry.corpus_counts.keys()) != {'ephrem_works'}:
            continue
...
def generate_batch(client, batch):
    ...
    user = (
        f"Here are {len(batch)} patterns from Ephrem's other hymn cycles. Identify the "
...
def main():
    ...
    targets = find_ephrem_works_exclusive_uncovered(extractor, cognates, args.min_occ)
    print(f"Existing cognate entries: {before}")
    print(f"Ephrem Works-exclusive uncovered roots (>= {args.min_occ} occ): {len(targets)}")
